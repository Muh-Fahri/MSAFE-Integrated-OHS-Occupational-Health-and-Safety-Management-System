        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-title-desc">Update the form below to edit menu.</p>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-10">
                                <input type="text" class="form-control @error('menu_name') is-invalid @enderror"
                                    id="menu_name" name="menu_name" value="{{ old('menu_name', $menu->menu_name) }}"
                                        ...
                                        <div class="mb-3">
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox" id="is_manual_override"
                                                    ...
                                                    {{ old('is_manual_override', $menu->is_manual_override) ? 'checked' : '' }}>
                                        ...
                                        <div id="actionsCheckboxes">
                                            @foreach ($allActions as $action)
                                                @php
                                                    $currentActions = old(
                                                        'available_actions',
                                                        $menu->getAvailableActions(),
                                                    );
                                                    $isChecked = in_array($action, $currentActions);
                                                @endphp
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                let detectedActions = [];
                let isManualOverride = {{ old('is_manual_override', $menu->is_manual_override) ? 'true' : 'false' }};
                ...
                // Warning jika mengubah parent menu yang punya children
                    ...
                    $('#parent_id').on('change', function() {
                        ...
                        if (parentId) {
                            Swal.fire({
                                icon: 'warning',
                                title: 'Warning!',
                                text: 'This menu has {{ $menu->children->count() }} child menu(s). Changing it to a child menu may affect the structure.',
                                showCancelButton: true,
                                confirmButtonText: 'Continue',
                                cancelButtonText: 'Cancel'
                            }).then((result) => {
                                if (!result.isConfirmed) {
                                    $(this).val('{{ $menu->parent_id }}');
                                    toggleIconField();
                                }
                            });
                        }
