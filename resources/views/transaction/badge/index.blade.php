    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between mb-3">
                <h4 class="mb-0">Badge Request</h4>
            </div>

            <div class="d-flex flex-wrap justify-content-between gap-2">
                <div class="{{ route('transaction-badge.create') }}flex flex-wrap gap-2">
                    <a href="{{ route('transaction-badge.create') }}" class="btn btn-sm btn-add">
                        <i class="fas fa-plus me-1"></i> Add
                    </a>
                    <button type="button" class="btn btn-sm btn-search" data-bs-toggle="modal" data-bs-target="#searchModal">
                        <i class="fas fa-search me-1"></i> Search
                    </button>
                    <a href="{{ route('transaction-badge.index', ['action' => 'REQUEST']) }}" class="btn btn-sm btn-request">
                        <i class="fas fa-paper-plane me-1"></i> Request
                    </a>
                    <a href="{{ route('transaction-badge.index', ['action' => 'APPROVAL']) }}" class="btn btn-sm btn-approval">
                        <i class="fas fa-check-double me-1"></i> Approval
                    </a>
                    <a href="{{ route('transaction-badge.index', ['action' => 'APPROVAL_HISTORY']) }}" class="btn btn-sm btn-appvHistory">
                        <i class="fas fa-history me-1"></i> Approval History
                    </a>
                    <a href="{{ route('transaction-badge.index', ['action' => 'PRINTING']) }}" class="btn btn-sm btn-printing">
                        <i class="fas fa-print me-1"></i> Printing
                    </a>
                    @if($is_able_to_monitor)
                    <a href="{{ route('transaction-badge.index', ['action' => 'MONITORING']) }}" class="btn btn-sm btn-monitoring">
                        <i class="fas fa-chart-line me-1"></i> Monitoring
                    </a>
                    @endif
                    <a href="{{ url('transaction/badge/export'.str_replace(Request::url(), '', Request::fullUrl())) }}" 
    ...
    <div class="modal fade" id="searchModal" tabindex="-1" aria-labelledby="searchModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="border-radius: 15px;">
                ...
                <form action="{{ route('transaction-badge.index') }}" method="GET">
                    <div class="modal-body">
                        <input type="hidden" name="action" value="{{ request('action') }}">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label small fw-bold">Request Date</label>
                                <input type="text" name="request_date" id="request_date" class="form-control rounded-pill" value="{{ request('request_date') }}" />
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold">Request No</label>
                                <input type="text" name="request_no" class="form-control rounded-pill" placeholder="123..." value="{{ request('request_no') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold">Status</label>
                                <select name="status" class="form-select rounded-pill">
                                    <option value="">-- All Status --</option>
                                    <option value="WAITING_TO_PRINT"{{ request('status') == 'WAITING_TO_PRINT' ? 'selected' : '' }}>WAITING_TO_PRINT</option>
                                    <option value="APPROVAL_REQUIRED"{{ request('status') == 'APPROVAL_REQUIRED' ? 'selected' : '' }}>APPROVAL_REQUIRED</option>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="requestor_name" class="form-control rounded-pill" placeholder="Requestor Name..." value="{{ request('requestor_name') }}">
                            </div>
                            <div class="col-12">
                    ...
                    <div class="modal-footer border-0">
                        <a href="{{ route('transaction-badge.index', ['action' => request('action')]) }}"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="datatable" class="table nowrap table-bordered dt-responsive w-100 align-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'request_date', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc']) }}"
                                            class="text-dark d-flex align-items-center justify-content-between text-decoration-none">
                                            Req. Date
                                            <i
                                                class="fas {{ request('sort_by') == 'request_date' ? (request('sort_order') == 'asc' ? 'fa-sort-up' : 'fa-sort-down') : 'fa-sort text-muted' }} ms-1"></i>
                                        </a>
                                    </th>
                                    <th>
                                    ...
                                    <th>
                                        <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'next_action', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc']) }}"
                                            ...
                                            Sub Company
                                            <i
                                                class="fas {{ request('sort_by') == 'sub_company_name' ? (request('sort_order') == 'asc' ? 'fa-sort-up' : 'fa-sort-down') : 'fa-sort text-muted' }} ms-1"></i>
                                        </a>
                                    </th>
                                    <th>
                            ...
                            <tbody>
                                @foreach ($badgesReq as $badge)
                                    <tr>
                                        <td class="text-center">
                                            <div class="d-flex gap-1 justify-content-center">
                                                {{-- View Button --}}
                                                <a href="{{ route('transaction-badge.show', $badge->id) }}"
                                                    class="btn btn-sm tombol-view" title="View">
                                                    View
                                                </a>

                                                {{-- Approval Button (Hanya jika type = APPROVAL) --}}
                                            </div>
                                        </td>
                                        <td>{{ $badge->request_no }}</td>
                                        <td>{{ $badge->request_date }}</td>
                                        <td>{{ $badge->company_name }}</td>
                                        <td>{{ $badge->sub_company_name }}</td>
                                        <td>{{ $badge->location }}</td>
                                        <td>{{ $badge->status }}</td>
                                        <td>{{ $badge->requestor_name }}</td>
                                        <td>{{ $badge->remarks }}</td>
                                        <td>{{ $badge->next_action }}</td>
                                        <td>{{ $badge->next_user_name }}</td>
                                        <td>{{ $badge->updated_at ? $badge->updated_at->format('Y-m-d H:i') : '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div class="text-muted small">
                            Showing <strong>{{ $badgesReq->firstItem() ?? 0 }}</strong> to
                            <strong>{{ $badgesReq->lastItem() ?? 0 }}</strong> of
                            <strong>{{ $badgesReq->total() }}</strong> entries
                        </div>

                        <nav>
                            <ul class="pagination pagination-sm mb-0">
                                {{-- Tombol Previous --}}
                                @if ($badgesReq->onFirstPage())
                                    <li class="page-item disabled">
                                        <span class="page-link shadow-none">Previous</span>
                                    </li>
                                @else
                                    <li class="page-item">
                                        <a class="page-link shadow-none" href="{{ $badgesReq->previousPageUrl() }}"
                                            rel="prev">Previous</a>
                                    </li>
                                @endif

                                {{-- Tombol Angka Halaman --}}
                                @foreach ($badgesReq->getUrlRange(max(1, $badgesReq->currentPage() - 2), min($badgesReq->lastPage(), $badgesReq->currentPage() + 2)) as $page => $url)
                                    <li class="page-item {{ $page == $badgesReq->currentPage() ? 'active' : '' }}">
                                ...
                                {{-- Tombol Next --}}
                                @if ($badgesReq->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link shadow-none" href="{{ $badgesReq->nextPageUrl() }}"
                                            rel="next">Next</a>
                                    </li>
                                @else
                                    <li class="page-item disabled">
